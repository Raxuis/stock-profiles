...
const ShinyButton = ({
  ...
  return (
    <motion.button
      ...
      onClick={() => {
        if (href) {
          router.push(href); // Ensure href is valid
        } else {
          console.error("Href is not provided or invalid");
        }
      }}
      className={cn(
    ...
    >
      <span
        className="relative flex size-full items-center justify-center text-sm uppercase tracking-wide text-[rgb(0,0,0,65%)] dark:font-light dark:text-[rgb(255,255,255,90%)]"
        style={{
          maskImage:
